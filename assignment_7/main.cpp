...
int main(void)
{
	...
	loadOBJ("ball-98fix.obj", vertices, elements);

	for (int i = 0; i < vertices.size(); i++) {
		colors.push_back(rand() / (float)RAND_MAX);
	}
	...
	MVP = MVP * glm::translate(glm::mat4(), glm::vec3(0.7f, 0.0f, 0.0f));
	while (!glfwWindowShouldClose(window))
	{
		...
		glm::rotate(glm::mat4(1.0f), glm::radians(angle), axis_y);
		MVP = MVP * anim;
